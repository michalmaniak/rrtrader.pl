...
if(!isset($_SESSION['login'])) // 2
	{
header("Location: /loguj.php?result=5", true, 301);
exit();
	}
	if($_SESSION['perm']==0)
{
...
<legend>Usuń oferty:</legend>
<form name="contactform" method="post" action="/remove.php">
...
<img id="captcha" src="/securimage/securimage_show.php" alt="CAPTCHA Image" /><br />
  ...
  Jaką ofertę chcesz skasować?</br>
<input type="checkbox" name="premium" value="premium_remove"  />  <label>Konto premium<br />
<input type="checkbox" name="gold" value="gold_remove" />  <label>Złoto<br />

	 <input type=submit value="Skasuj" onclick="clickHandler()"/>
<?php
$value = $_GET["result"];
...
{
			echo "<script>
	alert('Pomyślnie usunięto oferty');
			</script>";
}
else if($value == 4)
{
