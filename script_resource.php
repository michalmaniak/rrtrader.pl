...
if($oil=="Brak" & $ore=="Brak" & $uran=="Brak" & $diamond=="Brak")
{
	  ...
	  $query->execute();
header("Location: https://dev.rrtrader.tk/resource.php?result=1", true, 301);
exit();
}
else{
...
if($query->num_rows >= 1){
	   ...
	   $query = $conn->prepare("UPDATE `resources` SET `stone` = ?, `oil` = ?, `uran` = ?, `diamond` = ? WHERE `nick` = ? "); // prepate a query
$query->bind_param('sssss', $oil, $ore, $uran, $diamond, $_SESSION['nick'] ); // binding parameters via a safer way than via direct insertion into the query. 'i' tells mysql that it should expect an integer.
$query->execute();

header("Location: https://dev.rrtrader.tk/resource.php?result=2", true, 301);
exit();
}
...
$query->execute();
header("Location: https://dev.rrtrader.tk/resource.php?result=3", true, 301);
exit();
}
